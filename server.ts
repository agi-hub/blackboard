    ...
    try {
      ...
      if (path === "/api/poster" && req.method === "POST") {
        ...
        try {
          ...
          const styleZh = "简笔画，黑板报风格粉笔简笔画，手绘粉笔线条，线条轻微抖动不光滑，简笔，线条一定使用彩色粉笔，色彩表现在线条上，干净轮廓，一定不要填充，无阴影，无渐变，画面留白充足，2D 平面插画，高对比度，边缘干净，不要背景，纯黑背景。画面中绝对不要出现任何文字、字母、汉字、数字；只画图形，元素丰富一些，围绕主题安排多种小图形和装饰。";
          const styleEn = "simple sketch, blackboard bulletin chalk drawing, hand-drawn chalk lines, slightly shaky imperfect lines, minimal style, lines MUST use colored chalks (color lives in the lines only), clean outlines, STRICTLY NO FILLS, no shadows, no gradients, generous empty space, flat 2D illustration, high contrast, clean edges, no background, pure black background. ABSOLUTELY NO TEXT in the picture — no words, no letters, no characters, no numbers; pictures only. Make the composition rich with varied small motifs and decorations around the theme.";
          const prompt = en
            ? `${theme} themed blackboard bulletin. ${styleEn}`
            : `${theme}为主题的黑板报。${styleZh}`;

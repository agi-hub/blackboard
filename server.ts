...
function layoutSystemPrompt(W: number, H: number): string {
  return [
    ...
    "- 讲稿可嵌「板书动作标记」配合讲解：circle{词} = 讲到该词时在黑板上圈出它；underline{词} = 划下划线。词必须与该块板书 text 中的原文完全一致（便于定位）；每块讲稿 0~3 个标记；标记在转语音时会被剥离，不会读出。例：\"say\":\"我们重点看 circle{打分} 这一步，然后 underline{softmax} 归一化。\"",
    "",
    "排版规则：",

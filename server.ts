...
function layoutSystemPrompt(W: number, H: number): string {
  return [
    ...
    "- 图示独栏+大幅（硬性要求）：带 svg 的图示块必须独占一个完整大栏（≈700×660 整栏，只放图、不放文字块；同主题多张图可同栏纵向排）。严禁图示块与文字要点块混排同一 region，严禁给图开小栏/半栏。svg 本身要画满：元素铺满整个 viewBox（≥6 个元素），文字/线条大胆用空间——稀疏小图等于没画。",
    "- 有图页文字配额（硬性要求）：页面含 svg 图时，文字块总量 ≤ 3 块、每块 ≤ 2 行——图是主角，文字只留最核心要点；内容装不下就拆成更多页，绝不许压缩图来塞文字。",
